...
-----------------
function ns.CreateBorder(self, size, padding, layer, shadowLayer)
	...
	if not (self.borderTextures) then
		...
		for i = 1, #sections do 
			...
			t:SetParent(self)
			t:SetTexture(ns.config.bordertexture)
			t:SetVertexColor(1, 1, 1, 1)
			b[sections[i]] = t

			t = self:CreateTexture(nil, shadowLayer or 'BORDER')
			t:SetParent(self)
			t:SetTexture(ns.config.bordershadow)
			...
			t:SetParent(self)
			t:SetTexture(ns.config.borderglow)
